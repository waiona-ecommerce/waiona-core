...
Las imágenes se gestionan en el módulo `product-images`, independiente del producto pero ligado por FK. Cada imagen tiene una URL y un número de posición que determina el orden de visualización. La primera imagen (posición más baja) es la que aparece en el listado del shop.
...
Índice único compuesto: `(productId, position)` — no pueden existir dos imágenes con la misma posición en el mismo producto.
...
```typescript
{
  productId: number;   // int >= 1, el producto debe existir
  url: string;         // 5–255 chars, trimmed
  position: number;    // int >= 1
}
```

**Actualizar (`UpdateProductImageDto`):** `PartialType(CreateProductImageDto)` — todos los campos opcionales.
...
|---|---|---|---|
| `POST` | `/product-images` | Agrega una imagen al producto | ADMIN / SUPER_ADMIN |
| `GET` | `/product-images/product/:productId` | Lista todas las imágenes de un producto ordenadas por posición | ADMIN / SUPER_ADMIN |
| `GET` | `/product-images/:id` | Obtiene una imagen por ID | ADMIN / SUPER_ADMIN |
| `PATCH` | `/product-images/:id` | Actualiza URL o posición | ADMIN / SUPER_ADMIN |
| `DELETE` | `/product-images/:id` | Soft delete de la imagen | ADMIN / SUPER_ADMIN |
...
| Regla | Dónde se aplica |
|---|---|
| El producto debe existir antes de agregar imagen | `create()` → `NotFoundException` si el producto no existe |
...
```bash
npx jest --testPathPattern="products/product-images"
npx jest --config test/jest-e2e.json --testPathPattern="product-images.e2e"
```
