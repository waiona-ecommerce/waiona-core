...
describe('UsersController', () => {
  ...
  const mockUserDto = (overrides = {}) => ({
    id: 1, email: 'juan@test.com', isActive: true, role: 'client',
  ...
  describe('findAll', () => {
    it('should return paginated users', async () => {
      service.findAll.mockResolvedValue(mockPaginated() as any);
    ...
    it('should pass query params to service', async () => {
      ...
      expect(service.findAll).toHaveBeenCalledWith(query, query.page, query.limit);
    });
  });
  ...
  describe('findOne', () => {
    ...
    it('should throw ForbiddenException if accessing another user', () => {
      expect(() => controller.findOne(2, mockRequest(1))).toThrow(ForbiddenException);
    });
  });
  ...
  describe('update', () => {
    it('should update own user', async () => {
      ...
      const updated = mockUserDto({ profile: { id: 1, name: 'Carlos', lastName: 'Pérez', avatar: null } });
    ...
    it('should throw ForbiddenException if updating another user', () => {
      expect(() => controller.update(2, mockRequest(1), {} as any)).toThrow(ForbiddenException);
    });
  });
  ...
  describe('remove', () => {
    ...
    it('should throw ForbiddenException if removing another user', () => {
      expect(() => controller.remove(2, mockRequest(1))).toThrow(ForbiddenException);
    });
  });
});
